...
## Main concurrency simulator (current as of December 25 2025)
...
Added variable speed and noise parameters that would model baseline CPU capability and short term execution noise respectively. The speed variable is fixed for each worker, while the execution noise varies per time slice. Currently, the speed of each CPU varies from [0.8, 1.2] units of time and noise varies from [0.9, 1.1] units of time, which impacts how much work each CPU is capable of completing every time it is called by the scheduler. This implementation shows that even with these randomly varying parameters, my algorithm is still capable of appropriately scheduling tasks according to priority (if included)
